...
function SnakeStorage(){
    ...
    this.changeDirection = function(ownerId, direction){
        ...
        for (var i in this.storage)
        {
            if (this.storage[i].ownerId == ownerId && typeof(direction) !== 'undefined') {
                this.storage[i].snake.direction = direction;//new Point(unitVectorX, unitVectorY);
            }
        }
    };
    ...
    this.checkCollision = function(snakeToCheckId) {
        var collided = false;
        var snakeToCheck = this.get(snakeToCheckId);
        var snakesHeadsDistance;
        var snakesDistance;
        for(var i=0; i < this.storage.length; i++){
            if(snakeToCheckId != this.storage[i].ownerId){
                ...
                if(snakesHeadsDistance < 50){
                    ...
                    for(var j=0; j < this.storage[i].snake.tail.length; j++) {
